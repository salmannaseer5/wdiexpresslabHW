...
axios.get("http://localhost:8000/gifs").then(response => {
...
editSubmit.addEventListener("click", e => {
  ...
  axios
    .put(`http://localhost:8000/gifs/${currentlyEditing}`, {
      name,
      url
    })
    .then(resp => {
...
deleteButton.addEventListener("click", e => {
  // deletes an image
  axios.delete(`http://localhost:8000/gifs/${currentlyEditing}`).then(resp => {
    addPictures(resp.data);
    $("#modal-edit").modal("close");
  });
});
